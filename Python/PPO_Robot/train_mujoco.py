...
def train(agent, config, norm, metricsQue):
    ...
    if len(agent.replay_buffer) >= config.MIN_BUFFER_SIZE:
        ...
        norm_var = torch.tensor(norm.var, device=agent.device, dtype=torch.float32)
        norm_std = torch.sqrt(norm_var) + 1e-8

    
        for _ in range(config.GRADIENT_STEPS):
           
            buf = agent.replay_buffer.sample(config.BATCH_SIZE)
            # Pass the GPU tensors
...
def main():
    ...
    base_env = BirdBipedEnv(model_path="simulation/robot.xml")
